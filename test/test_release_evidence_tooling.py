...
def release_evidence(revision: str = "abc123") -> dict[str, object]:
    return {
        "status": "pass",
...
class ProductionArtifactContractTests(unittest.TestCase):
    def test_only_developer_id_application_is_release_identity(self) -> None:
        self.assertTrue(
            production.developer_id_identity(
    ...
    def test_release_codesign_requires_developer_id_and_secure_timestamp(self) -> None:
        ...
        self.assertFalse(
            production.codesign_is_release_ready(
                "Authority=Developer ID Application: Example (TEAMID)\n"
            )
        )
        self.assertFalse(
    ...
    def test_notary_submit_fails_closed_when_not_accepted(
        self, run: Mock
    ) -> None:
        run.return_value = Mock(
            stdout=json.dumps({"id": "submission", "status": "Invalid"})
        )
        with self.assertRaisesRegex(
            RuntimeError, "notarization was not accepted"
        ):
            production.notary_submit(Path("ClosedRoom.zip"), "profile", cwd=ROOT)

    @patch.object(production, "run")
    def test_restore_generated_frontend_is_bounded_to_static_output(
        self, run: Mock
    ) -> None:
        production.restore_generated_frontend(ROOT)
        target = "src/local_asr_server/static"
        self.assertEqual(
            run.call_args_list,
            [
                call(
                ...
                call(["git", "clean", "-fd", "--", target], cwd=ROOT),
            ],
        )
...
class MeasuredReleaseContractTests(unittest.TestCase):
    def test_production_manifest_rejects_non_notarized_app(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (directory / "build-manifest.json").write_text(
                json.dumps(
                    {
                        "configuration": {"signing": "identified"},
            ...
            with self.assertRaisesRegex(
                RuntimeError, "signing is not release-ready"
            ):
                measured.production_manifest_for(app)

    def test_production_manifest_requires_complete_release_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp)
            app = directory / "ClosedRoom.app"
            app.mkdir()
            ...
            with self.assertRaisesRegex(
                RuntimeError, "production-release-evidence"
            ):
                measured.production_manifest_for(app)

            (directory / "production-release-evidence.json").write_text(
                json.dumps({"status": "pass", "source_revision": "abc123"}),
                encoding="utf-8",
            )
            with self.assertRaisesRegex(RuntimeError, "missing release proof"):
                measured.production_manifest_for(app)
            ...
            self.assertEqual(
                payload["configuration"]["signing"],
                "developer-id-notarized",
            )

    def test_production_manifest_rejects_mismatched_release_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp)
            app = directory / "ClosedRoom.app"
            app.mkdir()
            ...
            (directory / "production-release-evidence.json").write_text(
                json.dumps(release_evidence("different")),
                encoding="utf-8",
            )
            with self.assertRaisesRegex(
                RuntimeError, "does not match build manifest"
            ):
                measured.production_manifest_for(app)
    ...
    def test_resource_sanitizer_keeps_only_bounded_metrics(self) -> None:
        payload = {
            ...
            "llm_sidecar": {
                "status": "ready",
                "current_rss_bytes": 300,
            },
            "heavy_workloads": {
                "status": "available",
                "active_count": 1,
                "queue_depth": 0,
                "active_by_type": {"transcription": 1},
                "pending": {"id": "private"},
            },
        ...
        self.assertEqual(
            result["heavy_active_by_type"], {"transcription": 1}
        )
    ...
    def test_thermal_sample_is_unknown_when_metrics_are_unavailable(
        self, run: Mock
    ) -> None:
    ...
    @patch.object(measured.subprocess, "run")
    def test_thermal_sample_accepts_explicit_nominal_state(
        self, run: Mock
    ) -> None:
        run.return_value = Mock(
        ...
        self.assertEqual(sample["state"], "nominal")
    ...
    def test_thermal_sample_parses_numeric_limits(self, run: Mock) -> None:
        ...
        sample = measured.thermal_sample()
        self.assertEqual(sample["status"], "available")
        self.assertEqual(sample["state"], "constrained")
        self.assertEqual(
            sample["metrics"]["cpu_scheduler_limit"], 100
        )
        self.assertEqual(sample["metrics"]["cpu_speed_limit"], 95)

    def test_audio_benchmark_completion_uses_prs9_schema(self) -> None:
        report = {
            "benchmark": "dual_track_vs_mixed_asr",
            "summary": {"repeat_count": 3},
            "runs": [{}, {}, {}],
        }
        self.assertTrue(measured.audio_benchmark_complete(report, 3))
        self.assertFalse(measured.audio_benchmark_complete(report, 2))
        self.assertFalse(
            measured.audio_benchmark_complete(
                {
                    "benchmark": "wrong",
                    "summary": {"repeat_count": 3},
                    "runs": [{}, {}, {}],
